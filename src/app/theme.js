...
const useTheme = () => {
  const theme = useSelector(state => state.user ? state.user.theme || 'light' : 'light');
  ...
  useEffect(() => {
    ...
    const themeObject = createMuiTheme(theme === 'dark' ? darktheme : lighttheme);
    ...
    css.setProperty('--background-color', themeObject.palette.background.default);
