...
class SyncButton extends Component {
  ...
  render() {
    ...
    if (badge > 9 && badge <= 99) { badgeStyle = classes.badge2digits; } else if (badge > 99) { badgeStyle = classes.badge3digits; }
      ...
      <div className={className}>
        <Tooltip title={`Last sync ${moment(last_sync).fromNow()}`} enterDelay={450} placement="bottom">
          <MenuItem disabled={isSyncing} onClick={() => { this.sync(); }}>
            <ListItemIcon>
              <Badge badgeContent={badge > 99 ? '99+' : badge} invisible={isSyncing || !badge} classes={{ badge: badgeStyle }} color="primary">
