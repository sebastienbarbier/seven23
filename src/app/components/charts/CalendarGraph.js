...
export default function CalendarGraph({
  ...
  useEffect(() => {
    if (values && values.length != array.length) {
      ...
      values &&
      !array.every((element, index) => element.date == values[index].date)
    ) {
      setArray(values);
    }
  }, [values]);
  ...
  const draw = (_svg) => {
    try {
      ...
      if (array || animateLoading) {
        let weeksCounter = 0;
        if (array && array.length) {
          weeksCounter = Math.min(
            moment
              .duration(
                moment(array[array.length - 1].date).diff(moment(array[0].date))
              )
              .as("weeks"),
            52
          );
        ...
        Calendar(_svg, array, {
          ...
          colors: d3.interpolateRgb.gamma(2.2)(
            d3.color(`${primaryColor}`),
            d3.color(`${primaryColor}20`)
          ),
        });
      }
    } catch (error) {
  ...
  function Calendar(
    ...
    if (animateLoading && data.length === 0) {
      ...
      const until = moment()
        .utc()
        .subtract(monthPerLine - 1, "months")
        .startOf("month")
        .toDate()
        .getTime();
      while (i >= until) {
    ...
    const duration_in_month = moment
      .duration(moment(data[data.length - 1].date).diff(moment(data[0].date)))
      .as("months");
    ...
    const date_end_last_row =
      X[years[years.length - 1][1][years[years.length - 1][1].length - 1]];
    const padding_for_last_row = timeWeek.count(
    ...
    const svg = _svg //d3.create("svg")
      ...
      .attr(
        "style",
        "max-width: 100%; min-width: 100%; height: auto; height: intrinsic;"
      )
    ...
    const cell = year
      ...
      .attr("x", (i, forloop_counter) => {
        if (forloop_counter == 0) {
          ...
          if (
            !isOneLineCalendar &&
            X[i].getTime() == earliest.toDate().getTime()
          ) {
        ...
        let x =
          (timeWeek.count(X[first_item], X[i]) + paddingCell) * cellSize + 0.5;
        return x;
      })
      ...
      .on("touchend", function (event) {
        ...
        if (
          Math.abs(touchstart.pageX - touchend.pageX) < 40 &&
    ...
    month
      ...
      .attr("d", (t, forloop_counter) => {
        ...
        if (forloop_counter == 0) {
          ...
          if (
            from.getFullYear() == earliest.toDate().getFullYear() &&
    ...
    month
      ...
      .attr("x", (d, forloop_counter) => {
        if (forloop_counter == 0) {
          ...
          // If ealiest is not first of month, we reajust.
          if (
            d.getFullYear() == earliest.toDate().getFullYear() &&
            ...
            if (d.getDate() != earliest.toDate().getDate()) {
              ...
              return (
                (timeWeek.count(first_item, d) + temporaryToRemove) * cellSize +
                0.5
              );
            }
          }
  ...
  const handleTooltipOpen = (event, delay = 0) => {
    const c = (
      <span
        onTouchEnd={(event) => {
          const el = Array.from(
            myRef.current.getElementsByClassName("selected")
          );
          if (el && el.length) {
            onClick(
              el[0].dataset.year,
    ...
    timeoutId.push(
      setTimeout(() => {
        Array.from(myRef.current.getElementsByClassName("selected")).forEach(
  ...
  const handleTooltipClose = (event) => {
    ...
    Array.from(myRef.current.getElementsByClassName("selected")).forEach((e) =>
