...
import { StyledTab, StyledTabs } from "./MonthLineWithControls/StyledTabs";
...
function generateData(account, statistics, theme, range = "ALL") {
  if (!statistics || !statistics.graph) {
    return [];
  }
  ...
  while (begin.isBefore(end.endOf("day"))) {
    ...
    if (step == "days") {
      ...
      stat =
        statistics.stats.perDates[begin.year()]?.months[begin.month()]?.days[
          begin.date()
        ];
    }
    if (stat) {
...
export default function MonthLineWithControls({
  ...
  const savedRange = useSelector((state) =>
    disableRangeSelector ? RANGE.length - 1 : state.dashboard.range
  );
  ...
  const [data, setData] = useState(() =>
    generateData(account, statistics, theme, dynamicRange ? "?" : null)
  );
    ...
    if (statistics) {
      setData(
        generateData(
          ...
          statistics,
          theme,
          dynamicRange ? "?" : RANGE[selectedRange]
        ).filter((_, i) => hiddenLines.indexOf(i) === -1)
      );
    }
