...
export default function Dashboard(props) {
  ...
  const show_migration_alert = useSelector(
  ...
  const show_has_unknown_amount = useSelector(
    (state) => statistics?.stats?.hasUnknownAmount
  );
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="paper transparent showMobile">
          <BalanceView isLoading={!statistics} statistics={statistics} />
        </div>
        ...
        <div className="paper pendings">
          <Typography variant="h6">
            Pending payments{" "}
            {!!statistics?.pendings.length && (
              <small>({statistics?.pendings.length})</small>
            )}
          </Typography>
        ...
        <div className="paper numbers">
          <Stack sx={{ pt: 1, pb: 1, height: "100%" }}>
            ...
            <Typography sx={{ pt: 1, pb: 2 }}>
              <strong>{account.name}</strong> contains:
            </Typography>
            <Typography>
              <span
                style={{
                  color: theme.palette.transactions.main,
                  fontWeight: 400,
                  fontSize: "1.1em",
                  paddingRight: 2,
                }}
            ...
            <Typography>
              <span
                style={{
                  color: theme.palette.changes.main,
                  fontWeight: 400,
                  fontSize: "1.1em",
                  paddingRight: 2,
                }}
              >
                {!changes ? <span className="loading w80" /> : changes.length}
              </span>{" "}
            ...
            <Typography>
              <span
                style={{
                  color: theme.palette.categories.main,
                  fontWeight: 400,
                  fontSize: "1.1em",
                  paddingRight: 2,
                }}
            ...
            <Typography sx={{ pt: 2, fontSize: "0.9em" }}>
              Duration:
              {!transactions ? (
                <span className="loading w80" />
              ) : (
                ` ${moment
                  .duration(
                    moment(account.oldest).diff(moment(account.youngest))
                  )
                  .humanize()}`
              )}
            </Typography>
            {server.isLogged && server.last_sync && (
              <Typography sx={{ pb: 2, fontSize: "0.9em" }}>
                Last sync: {moment(server.last_sync).fromNow()}
              </Typography>
            )}
          </Stack>
        </div>
