...
class Dashboard extends Component {
  ...
  handleToggleTrend = trend => {
    this.setState({
      component:
        trend && trend.component ? trend.component : this.state.component,
        ...
        <div className="layout_content">
          <div className={(openTrend ? "open" : "") + " trendModal"}>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <IconButton onClick={this.handleToggleTrend}>
                <Close color="action" />
              </IconButton>
            </div>
            {this.state.component}
                      ...
                      <p>
                        <span style={{ color: theme.palette.numbers.blue }}>
                          {!currentYear || isSyncing ? (
                            ...
                            <BalancedAmount
                              value={
                                currentYear.currentMonth.expenses +
                      ...
                      <p>
                        ...
                        <span style={{ color: theme.palette.numbers.green }}>
                          {!currentYear || isSyncing ? (
                            ...
                            <ColoredAmount
                              value={currentYear.currentMonth.incomes}
                              currency={selectedCurrency}
                            />
                          )}
                        </span>
                      ...
                      <p>
                        ...
                        <span style={{ color: theme.palette.numbers.red }}>
                          {!currentYear || isSyncing ? (
                            ...
                            <ColoredAmount
                              value={currentYear.currentMonth.expenses}
                              currency={selectedCurrency}
                            />
                          )}
                        </span>
                      ...
                      <p>
                        <span style={{ color: theme.palette.numbers.blue }}>
                          {!currentYear || isSyncing ? (
                            ...
                            <BalancedAmount
                              value={currentYear.expenses + currentYear.incomes}
                              currency={selectedCurrency}
                            />
                          )}
                        </span>
...
const mapStateToProps = (state, ownProps) => {
  const valid_until = state.user.profile
    ? state.user.profile.valid_until
    : new Date();
  return {
    subscription_expire_soon: moment().isBetween(
      moment(valid_until),
      moment().add(7, "days")
    ),
    ...
    changes_length:
      state.changes && state.changes.list ? state.changes.list.length : 0,
    user: state.user,
...
export default connect(mapStateToProps)(
  withTheme(withStyles(styles)(Dashboard))
);
